/*
 MY NOTES: On the website the functions are separated in the following sections: forEach, map, and filter.
 However, here in the code there were no such division. So, I started to code with the solution
 I thought fit better (using 'map', 'filter', regular expressions, etc.) These functions were kept here
 but were renamed by adding the suffix 'Map' to them, so they won't interfere in the tests.
 */
 ...
 */
function onlyEvenValuesMap(arr) {
 ...
 */
function addKeyAndValueMap(arr, key, value) {
  return arr.map((json, arr) => {
    json[key] = value;
    return json;
  });
}
...
function vowelCount(str) {
  ...
  vowelArray.forEach((val) => {
    cval             = countObject[val];
    countObject[val] = cval === undefined ? 1 : cval + 1;
  });
  return countObject;
}
...
function find(arr, searchValue) {
  // return arr.includes(searchValue) ? searchValue : undefined; // This works, too.
...
function removeVowels(str) {
  return Array.from(str).filter(val => val.match(/[^aeiou]/gi)).join('').toLowerCase();
}
